...
| D10 | **双流解耦**：嵌入（语义流，粗/低帧率）与 检测/跟踪/时空关系（感知流，密/高帧率）**分开取帧**，各自独立、互不绑定 | 二者对帧率需求不同；当前 MVA 用同一批稀疏帧（K=4/10s）把跟踪"饿死"。用户要求分开并留接口 |
| D11 | **离线优先**：现阶段不追求实时，聚焦对**录制好的本地视频**做 编码→检索→跟踪→问答；实时(P3)搁置 | 系统刚起步，用户明确 |
...
### 3.2 双流解耦：语义流（粗）与感知流（密）（D10 展开）

**动机**：embedding 是为了对一段视频有"整体语义理解"（粗粒度、低帧率够用）→ 服务检索/问答；跟踪与时空关系建模需要高帧率、细粒度。二者对帧率需求不同，**必须解耦**——否则用同一批稀疏帧（当前 MVA：K=4 帧/10s 段，且检测/跟踪与嵌入共用这 4 帧）会把跟踪"饿死"（这正是现状跟踪效果差的根因）。

**架构**：同一视频、共享**视频绝对时间基准**，拆成两条独立取帧流：

```
              同一视频（共享绝对时间基准）
        ┌──────────────┴──────────────┐
   语义流(粗)                        感知流(密) ← 新增、fps 可配
  段采样 K帧/window                高帧率取帧
    → 嵌入 → ChromaDB               → 检测 → 跟踪 → 跨视角关联 → 时空关系建模
    (检索 / 问答)                    (跟踪 / 关系)
```
- 两条流各自取帧、互不绑定，都按视频绝对时间写世界状态 → 检索到的段 `[t0,t1]` 与该窗口内的密集轨迹天然对齐。
- **感知流的 tracker、关系建模器置于可插拔接口后**（见 §3.1），后续换 ByteTrack/BoT-SORT/学习式 SGG 只改实现。

**要点 / 约束**：
- **源视频 fps 是硬上限**：低帧率源（如 Reservoir ~2.6 fps）再密也超不过源；跟踪要好需**高帧率源视频**。
- **算力**：密集检测远重于 4 帧/段；离线不追实时可接受，但**感知流 fps 必须可配**（能按需降密度）。
- **是"加"不是"重写"**：语义流（MVA 现有 ingest）不动，只**新增感知流**；把"检测/跟踪的取帧源"从"嵌入的段采样"里拆出。
- **跨视角关联**改跑在密集轨迹上（比跑段观测更准）。
- **接口先定、实现分步**：本轮先**定接口 + 做检索面板**（走语义流，数据已就绪）；**密集感知流 + 跟踪/关系随后**实现（跟踪面板待此完成后再做，见 §7）。

### 3.3 检索粒度（实测确认）
- 入库按 **10s 段**（window=stride=10s，**连续无间隔无重叠**）切分：每段 1 个段向量 + 段内每个检测目标 1 个 bbox 向量。（Reservoir 完整入库约 33 段；此前 8 段是 `--segments-per-view 4` 人为上限所致。）
- **两种检索粒度**：**段级(10s)** 与 **目标级(bbox)**。目标级带段内时间偏移 → **能定位到"某目标出现的那一帧"**（帧级）；段级到 10s 段起点。想让场景检索也到帧级，可调小 window 或逐帧编码（可配，代价更高）。
- **检索显示（定）**：一次查询给 **top-3 命中**（view/时刻/目标/分数），**缩略图只给 top-1**；点击命中 → 跳到对应视角该帧。
...
| 多视角检索 | 多视角信息检索 | 搜索(文/图)→**top-3 命中**(view/时刻/目标/分数) + **top-1 缩略图**；点击跳到对应视角该帧；显示检索透明化(查了 N 向量/命中 K)。粒度见 §3.3 | `/retrieve` | ✅ MVA 强项 |
...
> 注：本 pipeline 的 L1 检测/跟踪来自**感知流（密，高帧率）**（见 §3.2 双流解耦），与嵌入的段采样解耦；跟踪面板与本 pipeline 都待感知流实现后再做。
...
| 阶段 | 内容 | 交付 | 适用数据 |
|---|---|---|---|
| **P0 骨架** ✅ | MVA sidecar+`/health`+`/answer`+`/ingest`；OmniUAV `MvaClient`+引擎灯+问答路由降级；**进程内 ingest**+"入库"按钮；启动脚本自动读 key | 选视频→入库→**grounded 问答跑通**（已实测真实检测） | 本地视频 |
| **P1 检索 + 双流接口** ← 当前 | `/retrieve` + **多视角检索面板**(top-3 / 缩略图 top-1 / 点击跳帧 / 透明化)；**定义感知流(密)接口**(FrameSource / PerceptionPipeline / Tracker / RelationModeler 抽象+基线，不实现密集跑) | 离线本地视频：检索 + grounded 问答；感知流接口就位 | 本地视频 |
| **P1b 感知流+跟踪**（延后） | 实现密集感知流(fps 可配)→检测→跟踪→跨视角关联；跨视角跟踪面板 | 有效的跟踪可视化（需高帧率源视频） | 本地视频 |
| **P2 空间关系理解** | 基于感知流轨迹的 geo-grounding + 关系建模；`/state/relations`；空间关系面板；`/answer` 注入关系上下文 | 仿真/标定数据上**几何关系问答 + 关系列表可视化** | 仿真/标定 |
